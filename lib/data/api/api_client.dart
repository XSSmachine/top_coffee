...
class ApiClient extends GetConnect implements GetxService {
  ...
  ApiClient({required this.appBaseUrl, required this.sharedPreferences}) {
    ...
    groupId = sharedPreferences.getString(AppConstants.ACTIVE_GROUP) ?? "";
    _mainHeaders = {
  ...
  void updateHeader(String token, String groupId) {
    _mainHeaders = {
      'Content-type': 'application/json; charset=UTF-8',
      'Authorization': 'Bearer $token',
      'groupId': '$groupId'
  ...
  Future<http.Response> patchMultipart2(String uri, UpdateGroupModel body,
      {File? imageFile}) async {
    updateHeader(token, groupId);
    print('Sending request to: $uri');
    print('Sending request to GROUP ID: $groupId');
    var request = http.MultipartRequest('PATCH', Uri.parse(uri));

    request.fields["name"] = body.name.trim();
