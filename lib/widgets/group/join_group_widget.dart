...
class JoinGroupWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    void _login(AuthController authController){
      ...
      if (name.isEmpty) {
        showCustomSnackBar(
            "Type in group name", title: "Group name");
        ...
        authController.joinGroup(JoinGroup(name: name, password: password) ).then((status){
          if(status.isSuccess){
            showCustomSnackBar("Success joining your group",isError: false,title: "Success",color: Color(0xFF5669FF));
    ...
    return Column(
      children: [
        ...
        AppTextField(textController: groupNameController, hintText: "Group Name", icon: Icons.people_outline),
        SizedBox(height: Dimensions.height20,),
        AppTextField(textController: groupPasswordController, hintText: "Password", icon: Icons.lock_outline,isObscure: true,),
        SizedBox(height: Dimensions.height20*2,),
        GestureDetector(
          onTap: (){
            _login(controller);
